...
@synthesize name, description, angle;
...
- (void)setValue:(id)value forKey:(NSString*)key
{
	...
	if (self == [SampleObject sharedInstance])	return;
	
	// Class method - set last modified
	[SampleObject setLastModifiedKey:key forInstance:self];

	// Instance method - dummy setValue to dispatch notifications 
	[[SampleObject sharedInstance] setValue:self forKey:@"keyChanged"];
}
...
{
	static SampleObject* singleton;
	@synchronized(self)
	{
		if (!singleton)
			singleton = [[SampleObject alloc] init];
		return singleton;
	}
	return singleton;
}
...
//
static SampleObject* lastModifiedInstance;
...
- (SampleObject*)keyChanged
{
	return [SampleObject sharedInstance];
}
